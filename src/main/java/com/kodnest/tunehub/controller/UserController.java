...
public String validate(@RequestParam("email") String email,
	...
	if(userService.validUser(email,password)==true)
	{
		...
		// If valid, the email is stored in the session under the attribute name "email" to keep the user logged in.
		session.setAttribute("email",email);
		
		 // Get the role of the user (e.g., "admin" or "customer") by calling `userService.getRole`.
